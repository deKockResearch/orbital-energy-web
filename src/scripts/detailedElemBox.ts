...
function convertElectronConfigToHTML(): string {
  const aNumber = selectedElement$.get().selectedElementInfo?.number!;
  let i = 0;
  ...
  for (const orb of selectedElement$.get().selectedElemOrbitals!.slice(numOrbitalsInLastElem)) {
...
function displayDetailedElementBox(): void {
  ...
  if (row) {
    for (let detailsElemBox of detailsElemBoxes) {
      ...
      detailsElemBox.querySelector("#amassunc")!.textContent = "";
  ...
  for (let detailsElemBox of detailsElemBoxes) {
    ...
    detailsElemBox.querySelector("#aMass")!.textContent = String(selectedElement.aMass);
    ...
    detailsElemBox.querySelector("#amassunc")!.innerHTML = selectedElement.amassunc;
